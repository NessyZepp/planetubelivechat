...
import { registerCharLimiter }    from './common/lib/chat-limiter'
...
async function register (clientOptions: RegisterClientOptions): Promise<void> {
  ...
  registerHook({
    ...
    handler: () => {
      // wait a bit for the chat box to appear, then patch it
      const tries = 10;
      let count = 0;
      const timer = setInterval(() => {
        const ta = document.querySelector('textarea.chat-textarea') as HTMLTextAreaElement | null;
        if (ta || ++count >= tries) {
          clearInterval(timer);
        }
        if (ta) {
          ta.maxLength = 500;
          // optional: change placeholder so users know the limit
          ta.placeholder = (ta.placeholder || '').replace(/$/, ' (max 500 chars)');
        }
      }, 500);
    },
  });
